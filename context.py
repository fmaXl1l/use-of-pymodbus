...
class ModbusSlaveContext(IModbusSlaveContext):
    ...
    def setValues(self, fx, address, values):
        ...
        if not self.zero_mode:
            address = address + 1
        if values[0] == 0:
            #Codigo del medidor, etc
            value = 2.25    #valor del medidor 
            intPart = int(value)
            decimal = round((value - intPart) * 100) #dos decimales
            values[0] = intPart
            self.store[self.decode(fx)].setValues(address, values)
            values[0] = decimal
            self.store[self.decode(fx)].setValues(address + 1, values)
        elif values[0] == 1:
            value = "Hola"  #rfid
            cont = 0
            for character in value:
                values[0] = ord(character) #Ascii code
                self.store[self.decode(fx)].setValues(address + cont, values)
                cont = cont + 1
            values[0] = 0
            self.store[self.decode(fx)].setValues(address + cont, values)
        else:
